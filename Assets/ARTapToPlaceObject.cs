...
public class ARTapToPlaceObject : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (placementPoseIsValid && Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began) // prüft ob placementPoseIsValid && hat der User einen Finger auf dem Screen && checken ob die Touch-Phase dieses Fingers gerade begonnen hat
        {
            ...
            SoundManager.instance.PlaySingle(spawnSound); //hier nur single-sound - eigentlich ein Array der random mehrere Sounds spielen würde => SoundManager.instance.RandomizeSfx(spawnSound1, spawnSound2);
        }
    }

    private void PlaceObject()
    {
        Instantiate(objectToPlace, placementPose.position, placementPose.rotation); //spawne einen GameObject Clone (onjectToPlace) an Position und Rotation von placementPose
    }
    ...
    private void UpdatePlacementPose()
    {
        ...
        var hits = new List<ARRaycastHit>(); // speichert alle Hit-Points vom Raycast
